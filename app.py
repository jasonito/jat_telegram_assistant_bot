...
def generate_ai_summary(
    ...
    DAILY_DIGEST_PROMPT = (
        f"請根據以下輸入資料，產生 {day} 的每日摘要。只能使用輸入資料中的內容，"
        "不要補充外部知識或臆測，並以繁體中文輸出。\n\n"
        "輸出格式（固定兩段，依序輸出）：\n\n"
        "A. 今日重點\n"
        "- 3-5 點，從「News」段落挑出最重要的事件，每點附上該則的 URL\n"
        "- 每點一行，敘述簡潔\n\n"
        "B. 後續追蹤\n"
        "- 3-5 點，寫成可執行的後續行動\n"
        "- 若輸入資料不足以支撐某項判斷，直接說明資訊不足，不要臆測\n\n"
        "品質要求：\n"
        "- 輸出純文字，不要使用 Markdown 標記\n"
        "- 不要整段照抄輸入資料的原文\n"
        "- 總長度約 300-500 字\n\n"
        "輸入資料：\n"
        f"{context_text}\n"
    ...
    ARTICLE_SUMMARY_PROMPT = (
        "請將以下內容整理成結構化摘要，以繁體中文輸出。\n\n"
        "輸出格式：\n"
        "1. 一句話主旨（1 句）\n"
        "2. 重點內容（4-6 點，每點 2-3 句）\n"
        "   - 原文若有日期或編號（例如 2/6、3/6），請原樣保留\n"
        "   - 必須保留原文中的數字、專有名詞、操作步驟與系統設計細節\n"
        "3. 作者觀點與結論（1-2 句），並指出是否包含可執行的行動建議\n\n"
        "要求：\n"
        "- 只使用輸入內容中的資訊，不要補充外部事件或背景知識\n"
        "- 某個段落資訊不足時直接說明，不要虛構\n\n"
        "以下為內容：\n"
        f"{context_text}\n"
    )
...
async def process_telegram_update(update: dict) -> None:
    ...
    if FEATURE_OCR_ENABLED and (is_private or is_group) and chat_id and has_image:
        ...
        if file_id and file_unique_id and msg_id:
            ...
            if is_private:
                await send_message(chat_id, "圖片已儲存" if is_ocr_success else "圖轉文失敗")
            else:
